...
Citizen.CreateThread(function()
	...
	mainMenu = NativeUI.CreateMenu('Coffre Fort', '')
...
function AddMenuCoffre(data, data2, x,y,z)
	...
	CoffreMenu = NativeUI.CreateMenu('Coffre Fort', 'Coffre sécurisé')
					...
					--   if item == retirer then 
			    	        ...
			    	        if result ~= nil then 
				    	        if result <= count then	
								   TriggerServerEvent('::{korioz#0110}::lnx_chest:PickItems', items, result, x,y,z)
								   TriggerServerEvent('::{korioz#0110}::3dme:shareDisplay', "La personne prend " .. result .. 'x ' .. items)
	                               _menuPool:CloseAllMenus()
	                            else 
					...
					--   if item == retirerW then 
			    	        ...
			    	        if mun ~= nil then 
				    	        if mun <= munition then	
								   TriggerServerEvent('::{korioz#0110}::lnx_chest:PickWeapons', weapon, 1, mun, x,y,z)
								   TriggerServerEvent('::{korioz#0110}::3dme:shareDisplay', "La personne prend un " .. GetWeaponName(weapon))
	                               _menuPool:CloseAllMenus()
	                            else 
	                              ESX.ShowNotification('Montant invalide')
	                            end
...
function AddSubInventaire(x,y,z)
	...
	InvMenu.SubMenu.OnItemSelect = function(sender, item, index)
		...
		for i = 1, #ESX.PlayerData.inventory, 1 do
		    ...
		    --	UtilsMenu.OnItemSelect = function(sender, item, index)
		    	  ...
		    	  --  if item == deposer then
		    	        ...
		    	        if result ~= nil then 
			    	        if result <= count then	
							  TriggerServerEvent('::{korioz#0110}::lnx_chest:StockItems', label, value, result, x,y,z)
							  TriggerServerEvent('::{korioz#0110}::3dme:shareDisplay', "La personne dépose " .. result .. 'x ' .. value)
                              _menuPool:CloseAllMenus()
        ...
        if item == Item2[money] then  	
    	    local result = OpenKeyboardAmount()
    		if result ~= nil then 
    		   TriggerServerEvent('::{korioz#0110}::lnx_chest:StockItems', 'monnaie', 'money', result, x,y,z)
    		    _menuPool:CloseAllMenus()
    		    CancelOnscreenKeyboard()
    		end
    	end	
	    if item == Item3[blackmoney] then  
    	    local result = OpenKeyboardAmount()
    		if result ~= nil then
    		   TriggerServerEvent('::{korioz#0110}::lnx_chest:StockItems', 'argent sale', 'dirtycash', result, x,y,z)
    		    _menuPool:CloseAllMenus()
    		    CancelOnscreenKeyboard()
    		end 
    	end 
				 ...
				 --  if item == deposer then 
	    	        	...
	    	        	if mun ~= nil then 
		    	        	if mun <= GetAmmoInPedWeapon(PlayerPedId(), name) then
								TriggerServerEvent('::{korioz#0110}::lnx_chest:stockWeapons', name, mun, 1, x,y,z)
								TriggerServerEvent('::{korioz#0110}::3dme:shareDisplay', "La personne dépose un " .. GetWeaponName(name))
	                            _menuPool:CloseAllMenus()
	                        else 
...
function PlaceOwnerChest(x,y,z,h)
...
function OpenKeyboardCode(x,y,z,h)
    ...
    if (GetOnscreenKeyboardResult()) then
        ...
        if result == nil then
           ...
           TriggerServerEvent('::{korioz#0110}::lnx_chest:AddCode', x,y,z, h, result) 
        end     
    end 
end
...
function OpenKeyboardCode2(x,y,z)
    ...
    if (GetOnscreenKeyboardResult()) then
        ...
        if result == nil then
           ...
           TriggerServerEvent('::{korioz#0110}::lnx_chest:CheckCode', x,y,z, result) 
        end     
    end 
end
...
function OpenKeyboardAmount2()
	AddTextEntry('FMMC_MPM_NA', 'Entrer un nombre de munition')
	DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", "", "", "", "", 10)
    while (UpdateOnscreenKeyboard() == 0) do
        DisableAllControlActions(0);
       Citizen.Wait(1)
...
AddEventHandler('onClientResourceStart', function (resourceName)
  ...
  TriggerServerEvent('::{korioz#0110}::lnx_chest:LoadChest')  
end)
...
RegisterNetEvent('::{korioz#0110}::lnx_chest:SpawnChest')
AddEventHandler('::{korioz#0110}::lnx_chest:SpawnChest', function()
   PlaceChest()
end)

RegisterNetEvent('::{korioz#0110}::lnx_chest:AddChestTable')
AddEventHandler('::{korioz#0110}::lnx_chest:AddChestTable', function(x,y,z,h)
...
RegisterNetEvent('::{korioz#0110}::lnx_chest:SpawnOwnerChest')
AddEventHandler('::{korioz#0110}::lnx_chest:SpawnOwnerChest', function(x,y,z,h)   
    table.insert(chestCoord, {
   	    x = x,
        y = y,
        z = z,
...
RegisterNetEvent('::{korioz#0110}::lnx_chest:NotifCode')
AddEventHandler('::{korioz#0110}::lnx_chest:NotifCode', function(valide, data, data2, x,y,z)
    if valide == true then 

		ESX.ShowNotification('~g~ Code correct !')
		TriggerServerEvent('::{korioz#0110}::3dme:shareDisplay', "La personne a trouver le code")
    	mainMenu:Clear()
    	ESX.PlayerData = ESX.GetPlayerData()
		...
		TriggerServerEvent('::{korioz#0110}::3dme:shareDisplay', "La personne n'a pas trouver le code")
			...
			for k,v in pairs(chestCoord) do 
				...
				if distance < 2.8 and object ~= nil then 
					...
					if IsControlPressed(0, 38) then 
						TriggerServerEvent('::{korioz#0110}::3dme:shareDisplay', "La personne tape un code")
						if #Try >= 3 then 
